program map_editor
  ...
  if (trim(operation) == 'scale' .or. trim(operation) == 'add_offset' .or. &
       ...
       & trim(operation) == 'missing2mask' .or. trim(operation) == 'invert_mask' .or. &
       & trim(operation) == 'zero2mask') then
  ...
  else if (trim(operation) == 'ap_mask') then
     ...
     call initialize_single_map(mapname_in1, nside, ordering, nmaps, header, map)
     if (iargc() == 4) then
        call ap_mask(maskfile, nside, ordering, map)
     else if (iargc() == 5) then
        call ap_mask(maskfile, nside, ordering, map, fact)
     else
        write(*,*) ''
        write(*,*) "Usage: map_editor ap_mask [input map] [mask] [output map]"
        write(*,*) ''
     end if
     call write_result_map(mapname_out, nside, ordering, header, map, suffix=='_dp')
...
!      call summarize_detector_angles(mapname_in1, s_max, nside, mapname_out)
  ...
  ! else if (trim(operation) == 'copy_pol_to_pol') then

  !    call copy_pol_to_pol
...
contains
  ...
  subroutine print_help(option)
    ...
    if (trim(option) == 'scale' .or. trim(option) == 'add_offet' .or. &
       ...
       write(*,*) ''
       write(*,*) '   The print_two_maps_to_ascii reads a map and prints the pixel values'
       write(*,*) '   in ascii format to the output file.'
       write(*,*) ''
       write(*,*) '   Usage: map_editor print_two_maps_to_ascii [mapfile1] [mapfile2]'
       write(*,*) '                                              [mask] [mapfile out]'
       write(*,*) ''
       ...
       write(*,*) '   nside = 1, where map1 is a copy of one polarization of the input map.'
       write(*,*) ''
