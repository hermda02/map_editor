...
!   end subroutine fit_line
  ...
  subroutine fit_gain_offset
    ...
    do i = 1, maxiter
       ...
       !write(*,fmt='(a,i6,a,f8.4,a,f10.4)') 'iter = ', i, ', gain = ', gain(i), ', offset = ', offset(i)
...
!          stop
  ...
  subroutine ap_mask(maskfile, nside, ordering, map, fact)
    implicit none
...
!       call input_map(filenames(i), map, npix, nmaps)
  ...
  subroutine smooth_map(infile, r_fill, lmin, lmax, nside, ordering, nmaps, map, header, &
    ...
    integer(i4b),                               intent(inout)        :: r_fill
...
!     if (iargc() /= 5) then
!        write(*,*) 'Usage: map_editor median_filter_source_holes [inmap] [radius arcmin] [threshold] [outmap]'
!        stop
!     end if
...
!   end subroutine median_filter_misspix
  ...
  subroutine expand_mask
    ...
    if (iargc() /= 4) then
       write(*,*) 'Usage: map_editor expand_mask [mask_in] [radius arcmin] [mask_out]'
       stop
    end if
    ...
    ! Loop through all pixels, searching for holes (values less than 0.5 (zeros))
    allocate(listpix1(0:1000000), listpix2(0:1000000))
    mask = map
    do j = 1, nmaps
       do i = 0, npix-1
          ...
          call pix2vec_ring(nside, i, vec)
          call query_disc(nside, vec, radius, listpix1, n1) 
          !expand zeros in map/ set pixels within radius of zero (hole) to zero  
  ...
  subroutine fill_zero_mask_udgrade
    ...
    deallocate(map_in, map_out, mask, mask_lower, map_ud, maskzerocount, nside_lower)
  ...
  subroutine firas_calib(bandpass_in,weight_file,output_map) !,units)
    ...
    integer(i4b)       :: count,start,length,i,j,k,bands !,unit
...
4   close(31)
    ...
    do i=1,count
       k = 0
       do j=1,bands-1
          if ((firas(i+start) + 6.5d0) .GT. freq(j) .and. (firas(i+start) - 6.5d0) .LT. freq(j)) then
